...
def open_file():
    phone_book.clear()
    file = open(path, 'r', encoding='UTF-8')  # r - read, rb - открытие побайтево (работа с фото и музыкой)
    data = file.readlines()
    file.close()
    for contact in data:
...
def save_file():
    ...
    with open(path, 'w', encoding='UTF-8') as file:
        # w - write (перезапись), a - append (дополнение). Файл открыт в контесктном менеджере
        file.write(data)
